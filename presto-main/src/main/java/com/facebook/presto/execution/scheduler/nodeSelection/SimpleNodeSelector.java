        ...
        implements NodeSelector
{
    ...
    private List<InternalNode> getEligibleNodes(int limit, NodeMap nodeMap, List<RemoteTask> existingTasks)
    {
        List<InternalNode> existingNodes = existingTasks.stream()
                .filter(remoteTask -> remoteTask.getTaskStatus().getState() != TaskState.GRACEFUL_FAILED)
        ...
        boolean anyGracefulFailedTask = existingTasks.stream().anyMatch(remoteTask -> remoteTask.getTaskStatus().getState() == TaskState.GRACEFUL_FAILED);
