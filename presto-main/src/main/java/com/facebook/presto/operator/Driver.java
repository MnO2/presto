        ...
        if (allOperators.stream().anyMatch(operator -> operator instanceof HashAggregationOperator)) {
            if (!newSplits.isEmpty()) {
                ...
                if (!(split.getConnectorSplit() instanceof RemoteSplit)) {
                    if (newSplits.size() == 1) {
                        log.debug("HashAggregationOperator and split is one to one at table scan stage");
                    } else {
                        log.debug("HashAggregationOperator and split IS NOT one to one at table scan stage");
                    }
                }
            }
        }
